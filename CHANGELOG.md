...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/proxy-aiops` to
  `@zw008/proxy-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/proxy-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/proxy-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install proxy-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/proxy-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [proxy-aiops, uvx]` — either one suffices — with every variable kept
...
- **A failed read-back after a landed write is undetermined, not failed.** `set_server_weight` verifies the new weight by re-reading the server; when that read failed the whole call raised, auditing a change that *had* happened as an error and recording no undo token for it — the mirror image of reporting an unconfirmed write as a success. It now returns `outcomeUnknown` with the prior weight and the read-back error.
