"""``proxy-aiops secret`` — manage the encrypted credential store.

Secrets (proxy API credentials) are stored in ``~/.proxy-aiops/secrets.enc``
...
from proxy_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from proxy_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: proxy-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update PROXY_AIOPS_MASTER_PASSWORD.[/]")
