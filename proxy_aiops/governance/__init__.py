"""Governance harness for proxy-aiops — audit, policy, budget, undo, sanitize.

A self-contained, vendored governance layer. proxy-aiops has NO dependency on
any external skill family — this package is its own copy of the harness:
  ...
  - unified SQLite audit log under ``~/.proxy-aiops/`` (override with
    ``PROXY_AIOPS_HOME``).
  - ``sanitize`` — output hygiene (encoding-level defense) for API-returned text.

State lives under ``ops_home()`` (default ``~/.proxy-aiops``).
"""
