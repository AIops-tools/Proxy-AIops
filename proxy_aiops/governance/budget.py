...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``PROXY_RUNAWAY_MAX`` times within ``PROXY_RUNAWAY_WINDOW_SEC`` trips a
...
from proxy_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing proxy-specific; the decorator wraps it into a
