"""The ``@governed_tool`` decorator — mandatory wrapper for all proxy MCP tool functions.

Responsibilities:
  1. Pre-check: evaluate policy rules (deny, maintenance window)
  2. Execute: run the actual tool function
  3. Post-log: write audit record to ``~/.proxy-aiops/audit.db``
...
def governed_tool(
    ...
    *,
    ...
    """Decorator for all proxy MCP tool functions.
    ...
    Args:
        ...
        undo: Optional callable ``(params, result) -> dict | None`` returning an
            ...
            successful call the inverse is recorded to ~/.proxy-aiops/undo.db and the
...
class _CallState:
    ...
    def __init__(
        ...
        self.rationale = os.environ.get("PROXY_AUDIT_RATIONALE", "")
        self.approved_by = os.environ.get("PROXY_AUDIT_APPROVED_BY", "")
        self.risk_tier = ""
...
def _pre_check(state: _CallState) -> None:
    ...
    # review) are denied when none was recorded (PROXY_AUDIT_APPROVED_BY).
    ...
    if tier.requires_approver and not state.approved_by and not state.dry_run:
        reason = (
            ...
            f"Set PROXY_AUDIT_APPROVED_BY to the authorizing human (and "
            f"PROXY_AUDIT_RATIONALE to why) before retrying."
        )
        if tier.reason:
...
def _finalize(state: _CallState) -> None:
    ...
    # cancellation (cancelling mid-flight proxy calls is worse).
...
def _infer_skill(func: Any) -> str:
    ...
    ``proxy_aiops.ops.jobs`` → ``proxy-aiops``
    ``mcp_server.server`` → ``proxy-aiops`` (the only consumer here).
    """
    module = getattr(func, "__module__", "") or ""
    if module.startswith("proxy_aiops") or module.startswith("mcp_server"):
        return "proxy-aiops"
    return "unknown"
