"""Encrypted secret storage for proxy-aiops.

Secrets (proxy API credentials) are NEVER written to disk in plaintext. They
live in ``~/.proxy-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``PROXY_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "proxy-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "PROXY_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``PROXY_<TARGET>_SECRET``; ``prefix='PROXY_'`` and
