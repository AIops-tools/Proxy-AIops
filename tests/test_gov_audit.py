...
``proxy_aiops.governance.audit`` and binds state to an isolated home via
...
from proxy_aiops.governance.audit import (
    AuditEngine,
    _current_user,
    _safe_json,
    detect_agent,
...
def test_query_filters(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="proxy-aiops", tool="a", status="ok", workflow_id="w1")
    eng.log(skill="proxy-aiops", tool="b", status="error", workflow_id="w2")
    eng.log(skill="other", tool="a", status="ok", workflow_id="w1")

    assert len(eng.query()) == 3
    assert len(eng.query(skill="proxy-aiops")) == 2
...
def test_stats_aggregation(audit_home):
    eng = AuditEngine(audit_home / "audit.db")
    eng.log(skill="proxy-aiops", tool="a", status="ok")
    eng.log(skill="proxy-aiops", tool="b", status="error")
    ...
    assert stats["by_skill"] == {"proxy-aiops": 2, "other": 1}
